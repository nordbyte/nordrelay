...
- `CODEX_EXTERNAL_APPROVAL_SUDO_HELPER`: when `true`, NordRelay may fall back to `sudo -n python3` for trusted local Codex CLI approval input if same-user TTY injection is denied.
