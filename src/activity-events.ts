...
export function activityCategoryForType(type: string): WebActivityCategory {
  ...
  if (/^(user_|group_|telegram_chat_|telegram_link|discord_channel_|discord_link|permission_|access_|lock_)/.test(type)) return "security";
...
export function auditCategoryForAction(action: string): WebActivityCategory {
  ...
  if (/^(permission_|user_|group_|telegram_|discord_)/.test(action)) return "security";
