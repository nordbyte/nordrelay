// @ts-check
// Generated by scripts/generate-web-api-routes.mjs from src/web/web-api-contract.ts.
// Do not edit by hand.

/** @typedef {{ path: string; methods: import("../../../web-api-types.js").WebApiMethod[] } | { re: RegExp; methods: import("../../../web-api-types.js").WebApiMethod[] }} ApiRouteRule */

/** @type {ApiRouteRule[]} */
const WEB_API_CLIENT_ROUTE_RULES = [
  { path: "/api/auth/me", methods: ['GET'] },
  { path: "/api/dashboard/logout", methods: ['POST'] },
  { path: "/api/profile", methods: ['GET', 'PATCH'] },
  { path: "/api/profile/password", methods: ['POST'] },
  { path: "/api/profile/logout-other-sessions", methods: ['POST'] },
  { path: "/api/profile/mfa/totp/setup", methods: ['POST'] },
  { path: "/api/profile/mfa/totp/enable", methods: ['POST'] },
  { path: "/api/profile/mfa/totp/disable", methods: ['POST'] },
  { path: "/api/profile/mfa/recovery-codes", methods: ['POST'] },
  { path: "/api/profile/webauthn/register/options", methods: ['POST'] },
  { path: "/api/profile/webauthn/register/verify", methods: ['POST'] },
  { re: /^\/api\/profile\/webauthn\/[^\/]+$/, methods: ['DELETE'] },
  { path: "/api/profile/api-tokens", methods: ['GET', 'POST'] },
  { re: /^\/api\/profile\/api-tokens\/[^\/]+$/, methods: ['DELETE'] },
  { re: /^\/api\/profile\/sessions\/[^\/]+$/, methods: ['DELETE'] },
  { path: "/api/bootstrap", methods: ['GET'] },
  { path: "/api/health", methods: ['GET'] },
  { path: "/api/snapshot", methods: ['GET'] },
  { path: "/api/tasks", methods: ['GET'] },
  { path: "/api/progress", methods: ['GET'] },
  { path: "/api/metrics", methods: ['GET'] },
  { path: "/api/metrics/history", methods: ['GET'] },
  { path: "/api/metrics/observability", methods: ['GET'] },
  { path: "/api/jobs", methods: ['GET'] },
  { path: "/api/trace", methods: ['GET'] },
  { re: /^\/api\/jobs\/[^\/]+\/log$/, methods: ['GET'] },
  { re: /^\/api\/jobs\/[^\/]+\/action$/, methods: ['POST'] },
  { path: "/api/active-sessions", methods: ['GET'] },
  { path: "/api/version", methods: ['GET'] },
  { path: "/api/update", methods: ['POST'] },
  { path: "/api/agent-updates", methods: ['GET'] },
  { path: "/api/agent-update", methods: ['POST'] },
  { re: /^\/api\/agent-update\/[^\/]+\/log$/, methods: ['GET', 'DELETE'] },
  { re: /^\/api\/agent-update\/[^\/]+\/input$/, methods: ['POST'] },
  { re: /^\/api\/agent-update\/[^\/]+\/cancel$/, methods: ['POST'] },
  { path: "/api/adapters/health", methods: ['GET'] },
  { path: "/api/adapters/conformance", methods: ['GET'] },
  { path: "/api/peers", methods: ['GET', 'POST'] },
  { path: "/api/peers/invite", methods: ['POST'] },
  { path: "/api/peers/pair", methods: ['POST'] },
  { path: "/api/peers/probe", methods: ['POST'] },
  { path: "/api/peers/discover", methods: ['GET'] },
  { path: "/api/peers/discovery-jobs", methods: ['GET', 'POST'] },
  { path: "/api/peers/relay", methods: ['GET', 'POST'] },
  { re: /^\/api\/peers\/discovery-jobs\/[^\/]+$/, methods: ['GET'] },
  { re: /^\/api\/peers\/discovery-jobs\/[^\/]+\/cancel$/, methods: ['POST'] },
  { re: /^\/api\/peers\/discovery-jobs\/[^\/]+\/log$/, methods: ['GET'] },
  { path: "/api/peers/identity/backup", methods: ['GET'] },
  { path: "/api/peers/identity/restore", methods: ['POST'] },
  { path: "/api/peers/global-sessions", methods: ['GET'] },
  { path: "/api/peers/sync", methods: ['POST'] },
  { re: /^\/api\/peers\/invitations\/[^\/]+$/, methods: ['DELETE'] },
  { re: /^\/api\/peers\/[^\/]+\/sync-candidates$/, methods: ['GET'] },
  { re: /^\/api\/peers\/[^\/]+\/sync-invite$/, methods: ['POST'] },
  { re: /^\/api\/peers\/[^\/]+\/repin$/, methods: ['POST'] },
  { re: /^\/api\/peers\/[^\/]+\/rotate$/, methods: ['POST'] },
  { re: /^\/api\/peers\/[^\/]+\/health$/, methods: ['GET'] },
  { re: /^\/api\/peers\/[^\/]+\/debug$/, methods: ['GET'] },
  { re: /^\/api\/peers\/[^\/]+\/debug\/probe$/, methods: ['POST'] },
  { re: /^\/api\/peers\/[^\/]+\/effective-access$/, methods: ['GET'] },
  { re: /^\/api\/peers\/[^\/]+\/health-history$/, methods: ['GET'] },
  { re: /^\/api\/peers\/[^\/]+\/repair$/, methods: ['POST'] },
  { re: /^\/api\/peers\/[^\/]+$/, methods: ['PATCH', 'DELETE'] },
  { re: /^\/api\/peers\/[^\/]+\/proxy$/, methods: ['POST'] },
  { re: /^\/api\/peers\/[^\/]+\/events$/, methods: ['GET'] },
  { path: "/api/permissions", methods: ['GET'] },
  { path: "/api/users", methods: ['GET', 'POST'] },
  { re: /^\/api\/users\/[^\/]+$/, methods: ['PATCH'] },
  { re: /^\/api\/users\/[^\/]+\/password$/, methods: ['POST'] },
  { re: /^\/api\/users\/[^\/]+\/sessions$/, methods: ['GET', 'DELETE'] },
  { re: /^\/api\/users\/[^\/]+\/sessions\/[^\/]+$/, methods: ['DELETE'] },
  { re: /^\/api\/users\/[^\/]+\/telegram$/, methods: ['POST'] },
  { re: /^\/api\/users\/[^\/]+\/telegram\/[^\/]+$/, methods: ['DELETE'] },
  { re: /^\/api\/users\/[^\/]+\/discord$/, methods: ['POST'] },
  { re: /^\/api\/users\/[^\/]+\/discord\/[^\/]+$/, methods: ['DELETE'] },
  { re: /^\/api\/users\/[^\/]+\/slack$/, methods: ['POST'] },
  { re: /^\/api\/users\/[^\/]+\/slack\/[^\/]+$/, methods: ['DELETE'] },
  { re: /^\/api\/users\/[^\/]+\/matrix$/, methods: ['POST'] },
  { re: /^\/api\/users\/[^\/]+\/matrix\/[^\/]+$/, methods: ['DELETE'] },
  { path: "/api/groups", methods: ['GET', 'POST'] },
  { re: /^\/api\/groups\/[^\/]+$/, methods: ['PATCH'] },
  { path: "/api/telegram-chats", methods: ['GET', 'POST'] },
  { re: /^\/api\/telegram-chats\/[^\/]+$/, methods: ['PATCH'] },
  { path: "/api/discord-channels", methods: ['GET', 'POST'] },
  { re: /^\/api\/discord-channels\/[^\/]+$/, methods: ['PATCH'] },
  { path: "/api/slack-channels", methods: ['GET', 'POST'] },
  { re: /^\/api\/slack-channels\/[^\/]+$/, methods: ['PATCH'] },
  { path: "/api/matrix-rooms", methods: ['GET', 'POST'] },
  { re: /^\/api\/matrix-rooms\/[^\/]+$/, methods: ['PATCH'] },
  { path: "/api/audit", methods: ['GET'] },
  { path: "/api/locks", methods: ['GET', 'POST', 'DELETE'] },
  { path: "/api/auth/status", methods: ['GET'] },
  { path: "/api/auth/login", methods: ['POST'] },
  { path: "/api/auth/logout", methods: ['POST'] },
  { re: /^\/api\/approvals\/[^\/]+\/respond$/, methods: ['POST'] },
  { path: "/api/settings", methods: ['GET', 'PATCH'] },
  { path: "/api/settings/wizard/test", methods: ['POST'] },
  { path: "/api/plugins", methods: ['GET', 'POST'] },
  { path: "/api/plugins/catalog", methods: ['GET'] },
  { path: "/api/plugins/marketplace", methods: ['GET'] },
  { path: "/api/plugins/analyze", methods: ['POST'] },
  { path: "/api/plugins/validate", methods: ['POST'] },
  { path: "/api/plugins/scaffold", methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+$/, methods: ['GET', 'DELETE'] },
  { re: /^\/api\/plugins\/[^\/]+\/enable$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/disable$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/settings$/, methods: ['PATCH'] },
  { re: /^\/api\/plugins\/[^\/]+\/log$/, methods: ['GET'] },
  { re: /^\/api\/plugins\/[^\/]+\/events$/, methods: ['GET'] },
  { re: /^\/api\/plugins\/[^\/]+\/manifest$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/update-check$/, methods: ['GET'] },
  { re: /^\/api\/plugins\/[^\/]+\/update$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/jobs$/, methods: ['GET', 'POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/jobs\/[^\/]+$/, methods: ['GET'] },
  { re: /^\/api\/plugins\/[^\/]+\/jobs\/[^\/]+\/cancel$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/rollback$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/invoke$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/command$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/aggregate-command$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/panel$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/artifact-handler$/, methods: ['POST'] },
  { re: /^\/api\/plugins\/[^\/]+\/diagnostics$/, methods: ['GET'] },
  { re: /^\/api\/plugins\/[^\/]+\/collector$/, methods: ['POST'] },
  { path: "/api/projects", methods: ['GET', 'POST'] },
  { re: /^\/api\/projects\/jobs\/[^\/]+\/cancel$/, methods: ['POST'] },
  { re: /^\/api\/projects\/[^\/]+$/, methods: ['GET', 'PATCH', 'DELETE'] },
  { re: /^\/api\/projects\/[^\/]+\/sessions$/, methods: ['POST'] },
  { re: /^\/api\/projects\/[^\/]+\/sessions\/[^\/]+$/, methods: ['DELETE'] },
  { re: /^\/api\/projects\/[^\/]+\/summary$/, methods: ['PATCH'] },
  { re: /^\/api\/projects\/[^\/]+\/summary\/run$/, methods: ['POST'] },
  { re: /^\/api\/projects\/[^\/]+\/summary\/history$/, methods: ['GET'] },
  { re: /^\/api\/projects\/[^\/]+\/summary\/history\/[^\/]+$/, methods: ['GET', 'PATCH', 'DELETE'] },
  { re: /^\/api\/projects\/[^\/]+\/summary\/history\/[^\/]+\/restore$/, methods: ['POST'] },
  { re: /^\/api\/projects\/[^\/]+\/plan$/, methods: ['PATCH'] },
  { re: /^\/api\/projects\/[^\/]+\/plan\/run$/, methods: ['POST'] },
  { re: /^\/api\/projects\/[^\/]+\/plan\/history$/, methods: ['GET'] },
  { re: /^\/api\/projects\/[^\/]+\/plan\/history\/[^\/]+$/, methods: ['GET', 'PATCH', 'DELETE'] },
  { re: /^\/api\/projects\/[^\/]+\/plan\/history\/[^\/]+\/restore$/, methods: ['POST'] },
  { re: /^\/api\/projects\/[^\/]+\/jobs$/, methods: ['GET'] },
  { path: "/api/templates", methods: ['GET', 'POST'] },
  { path: "/api/templates/import", methods: ['POST'] },
  { re: /^\/api\/templates\/[^\/]+\/versions$/, methods: ['GET'] },
  { re: /^\/api\/templates\/[^\/]+\/diff$/, methods: ['GET'] },
  { re: /^\/api\/templates\/[^\/]+\/export$/, methods: ['GET'] },
  { re: /^\/api\/templates\/[^\/]+\/versions\/[^\/]+\/export$/, methods: ['GET'] },
  { re: /^\/api\/templates\/[^\/]+\/versions\/[^\/]+\/rollback$/, methods: ['POST'] },
  { re: /^\/api\/templates\/[^\/]+\/versions\/[^\/]+\/run$/, methods: ['POST'] },
  { re: /^\/api\/templates\/[^\/]+\/versions\/[^\/]+\/preview$/, methods: ['POST'] },
  { re: /^\/api\/templates\/[^\/]+$/, methods: ['PUT', 'DELETE'] },
  { re: /^\/api\/templates\/[^\/]+\/run$/, methods: ['POST'] },
  { re: /^\/api\/templates\/[^\/]+\/preview$/, methods: ['POST'] },
  { path: "/api/workflows", methods: ['GET', 'POST'] },
  { path: "/api/workflows/import", methods: ['POST'] },
  { re: /^\/api\/workflows\/[^\/]+\/versions$/, methods: ['GET'] },
  { re: /^\/api\/workflows\/[^\/]+\/diff$/, methods: ['GET'] },
  { re: /^\/api\/workflows\/[^\/]+\/export$/, methods: ['GET'] },
  { re: /^\/api\/workflows\/[^\/]+\/versions\/[^\/]+\/export$/, methods: ['GET'] },
  { re: /^\/api\/workflows\/[^\/]+\/versions\/[^\/]+\/rollback$/, methods: ['POST'] },
  { re: /^\/api\/workflows\/[^\/]+\/versions\/[^\/]+\/run$/, methods: ['POST'] },
  { re: /^\/api\/workflows\/[^\/]+\/versions\/[^\/]+\/preview$/, methods: ['POST'] },
  { re: /^\/api\/workflows\/[^\/]+\/dry-run$/, methods: ['POST'] },
  { re: /^\/api\/workflows\/[^\/]+\/triggers$/, methods: ['GET', 'POST'] },
  { re: /^\/api\/workflows\/[^\/]+\/triggers\/[^\/]+$/, methods: ['DELETE'] },
  { re: /^\/api\/workflows\/[^\/]+$/, methods: ['PUT', 'DELETE'] },
  { re: /^\/api\/workflows\/[^\/]+\/run$/, methods: ['POST'] },
  { re: /^\/api\/workflows\/[^\/]+\/preview$/, methods: ['POST'] },
  { re: /^\/api\/workflow-triggers\/[^\/]+\/run$/, methods: ['POST'] },
  { re: /^\/api\/workflow-runs\/[^\/]+$/, methods: ['GET'] },
  { re: /^\/api\/workflow-runs\/[^\/]+\/report$/, methods: ['GET'] },
  { re: /^\/api\/workflow-runs\/[^\/]+\/cancel$/, methods: ['POST'] },
  { re: /^\/api\/workflow-runs\/[^\/]+\/rerun-failed$/, methods: ['POST'] },
  { path: "/api/control-options", methods: ['GET'] },
  { path: "/api/sessions", methods: ['GET'] },
  { path: "/api/sessions/new", methods: ['POST'] },
  { path: "/api/sessions/worktrees", methods: ['GET'] },
  { path: "/api/sessions/worktrees/fork", methods: ['POST'] },
  { path: "/api/sessions/worktrees/compare", methods: ['POST'] },
  { path: "/api/sessions/worktrees/integrate", methods: ['POST'] },
  { path: "/api/sessions/worktrees/integrate/preview", methods: ['POST'] },
  { path: "/api/sessions/worktrees/integrate/patch", methods: ['POST'] },
  { path: "/api/sessions/worktrees/cleanup", methods: ['POST'] },
  { re: /^\/api\/sessions\/worktrees\/integrations\/[^\/]+\/finalize$/, methods: ['POST'] },
  { re: /^\/api\/sessions\/worktrees\/[^\/]+\/diff$/, methods: ['GET'] },
  { re: /^\/api\/sessions\/worktrees\/[^\/]+\/update$/, methods: ['POST'] },
  { re: /^\/api\/sessions\/worktrees\/[^\/]+\/commit$/, methods: ['POST'] },
  { re: /^\/api\/sessions\/worktrees\/[^\/]+$/, methods: ['DELETE'] },
  { path: "/api/sessions/switch", methods: ['POST'] },
  { path: "/api/sessions/attach", methods: ['POST'] },
  { path: "/api/sessions/detail", methods: ['GET'] },
  { path: "/api/sessions/name", methods: ['POST'] },
  { path: "/api/agent", methods: ['POST'] },
  { path: "/api/models", methods: ['GET'] },
  { path: "/api/session/model", methods: ['POST'] },
  { path: "/api/session/reasoning", methods: ['POST'] },
  { path: "/api/session/fast", methods: ['POST'] },
  { path: "/api/session/launch", methods: ['POST'] },
  { path: "/api/prompt", methods: ['POST'] },
  { path: "/api/prompt/upload", methods: ['POST'] },
  { path: "/api/abort", methods: ['POST'] },
  { path: "/api/stop", methods: ['POST'] },
  { path: "/api/handback", methods: ['POST'] },
  { path: "/api/retry", methods: ['POST'] },
  { path: "/api/sync", methods: ['POST'] },
  { path: "/api/queue", methods: ['GET', 'POST'] },
  { path: "/api/queue/plans", methods: ['GET', 'POST'] },
  { re: /^\/api\/queue\/plans\/[^\/]+$/, methods: ['PATCH', 'DELETE'] },
  { re: /^\/api\/queue\/plans\/[^\/]+\/move$/, methods: ['POST'] },
  { re: /^\/api\/queue\/plans\/[^\/]+\/approve$/, methods: ['POST'] },
  { re: /^\/api\/queue\/plans\/[^\/]+\/enqueue$/, methods: ['POST'] },
  { path: "/api/chat/history", methods: ['GET', 'DELETE'] },
  { path: "/api/chat/attachment", methods: ['GET'] },
  { path: "/api/chat/mirror", methods: ['GET', 'POST'] },
  { path: "/api/activity", methods: ['GET'] },
  { path: "/api/artifacts", methods: ['GET', 'DELETE'] },
  { path: "/api/artifacts/usage", methods: ['GET'] },
  { path: "/api/artifacts/cleanup/preview", methods: ['POST'] },
  { path: "/api/artifacts/cleanup/run", methods: ['POST'] },
  { path: "/api/artifacts/bulk", methods: ['POST'] },
  { path: "/api/artifacts/zip", methods: ['GET'] },
  { path: "/api/artifacts/file", methods: ['GET'] },
  { path: "/api/artifacts/preview", methods: ['GET'] },
  { path: "/api/artifacts/diff", methods: ['GET'] },
  { path: "/api/logs", methods: ['GET'] },
  { path: "/api/logs/clear", methods: ['POST'] },
  { path: "/api/diagnostics", methods: ['GET'] },
  { path: "/api/diagnostics/voice/refresh", methods: ['POST'] },
  { path: "/api/diagnostics/bundle", methods: ['GET'] },
  { path: "/api/doctor", methods: ['GET'] },
  { path: "/api/doctor/fix", methods: ['POST'] },
  { path: "/api/runtime/restart", methods: ['POST'] },
];

/** @type {{ NORDRELAY_WEB_API_CLIENT_ROUTE_RULES?: ApiRouteRule[] }} */ (globalThis).NORDRELAY_WEB_API_CLIENT_ROUTE_RULES = WEB_API_CLIENT_ROUTE_RULES;
