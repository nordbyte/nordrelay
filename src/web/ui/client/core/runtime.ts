const state: DashboardState = { snapshot:null, controls:null, newSessionControls:null, enabledAgents:[], auth:null, profile:null, csrfToken:null, apiStatus:{local:defaultApiStateEntry('local'),peers:{}}, authReloading:false, permissions:[], settings:[], settingsDraft:{}, settingsErrors:{}, settingsSearch:'', settingsOpenCategories:{}, currentPage:'overview', settingsGroup:null, settingsWizard:null, accessTab:'users', adapterTab:'adapters', pluginTab:'installed', peerTab:'status', workflowTab:'templates', queueTab:'queue', sessionTab:'sessions', monitorTab:'activity', diagnosticsTab:'overview', queuePlanner:null, workflowTemplates:[], workflows:[], workflowRuns:[], logsPlain:'', logTimer:null, toastTimer:null, stickyToastActive:false, stickyToastText:'', cliStatusActive:false, webMirror:null, selectedArtifactTurns:new Set(), mediaRecorder:null, recordedChunks:[], events:null, reconnectTimer:null, notifications:false, completionSound:false, completionSoundAudioContext:null, completionSoundArmedKey:null, toolTooltipTimer:null, toolTooltipTarget:null, toolsVisible:false, themePreference:null, agentUpdateJobs:[], versionRequestId:0, sessionsRequestId:0, sessionAgeTimer:null, activityAgeTimer:null, chatWorkingTimer:null, sessionDetailRefreshTimer:null, sessionDetailAgeTimer:null, sessionDetailThreadId:null, sessionDetailAgentId:null, sessionDetailPeerId:null, sessionDetailRequestId:0, chatHistoryRequestId:0, chatRenderVersion:0, chatHistoryPagination:null, chatHistoryLoadingOlder:false, chatTabs:[], activeChatTabId:localStorage.getItem('nordrelayActiveChatTabId')||'', activeSessions:null, activeSessionsTimer:null, activeSessionDurationTimer:null, activeSessionsLoading:false, activeSessionsLastLoadAt:0, activeSessionsPeerBackoff:{}, activeSessionsTarget:localStorage.getItem('nordrelayActiveSessionsTarget')||'local', localTurnThreadId:null, localTurnAgentId:null, localTurnPeerId:null, localTurnStartedAt:null, peers:null, peerRelay:null, peerRefreshTimer:null, peerInviteSecrets:{}, peerProbeResult:null, peerDiscoveryJobs:[], incrementalRenders:{}, selectedPeer:localStorage.getItem('nordrelayPeerTarget')||'local' };
...
function applyPermissions(){
  ...
  const disableMap=[
    ...
    ['#installPluginBtn,#validatePluginSourceBtn,#createPluginScaffoldBtn,[data-plugin-remove],[data-plugin-reload]','plugins.install'],
...
function pageTitleLabel(name){return PAGE_LABELS[name]||name[0].toUpperCase()+name.slice(1)}
...
function page(name,options:WebuiPersistOptions={}){if(state.currentPage==='chat')saveActiveChatTabDraft();name=normalizePageName(name);state.currentPage=name;if(options.persist!==false){localStorage.setItem(PAGE_STORAGE_KEY,name);if(location.hash.slice(1)!==name)history.replaceState(null,'','#'+name)}if(name!=='sessions')stopSessionAgeCounter();if(name!=='peers')stopPeerTableRefresh();syncActiveSessionsRefresh();openSectionForPage(name);document.querySelectorAll('nav button[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===name));syncNavSections();document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id==='page-'+name));renderPageTitle(name);setMobileMenuOpen(false); void reloadCurrentPage().catch(handleUiError);}
async function reloadCurrentPage(options:WebuiReloadPageOptions={}){const name=state.currentPage;if(name==='overview') await loadActiveSessions(); if(name==='chat'){await ensureActiveChatTabSelected();const [historyRendered]=await Promise.all([loadChatHistory({forceScroll:true}),loadMirrorPreference(),loadActiveSessions()]);renderChatTabs();const input=document.getElementById('promptInput');if(input&&!input.value)restoreActiveChatTabDraft();if(historyRendered)scrollChatToBottom({force:true})} if(name==='workflows') await loadWorkflows(); if(name==='sessions'){await loadSessions(true,options.agentId);if(state.sessionTab==='worktrees')await loadWorktrees()} if(name==='queue') await loadQueue(); if(name==='monitor') await loadMonitor(); if(name==='settings') await loadSettings(); if(name==='logs') await loadLogs(); if(name==='diagnostics') await loadDiagnostics(); if(name==='metrics') await loadMetrics(); if(name==='adapters') await loadAdapterHealth(); if(name==='peers') await loadPeers(); if(name==='plugins') await loadPlugins(); if(name==='access') await loadAccess(); if(name==='version') await loadVersion();}
