...
function taskCard(t,title){
  ...
  return '<div class="item"><strong>'+esc(title+' · '+t.status)+'</strong><small>'+esc((t.agentLabel||t.agentId||t.source)+' / '+(t.threadId||'-'))+'</small>'+(t.correlationId?'<small>CID: <button type="button" class="copy-id" data-copy-id="'+attr(t.correlationId)+'">'+esc(t.correlationId)+'</button> <button type="button" class="secondary mini-button" data-trace-id="'+attr(t.correlationId)+'">Trace</button></small>':'')+'<small>'+esc('Elapsed '+fmtDuration(t.durationMs)+' / current '+(t.currentTool||'-')+' / last '+(t.lastTool||'-'))+'</small><small>'+esc('Tools: '+tools+' / output chars '+(t.outputChars||0))+'</small><small>'+esc(t.prompt||t.detail||'')+'</small></div>';
}
