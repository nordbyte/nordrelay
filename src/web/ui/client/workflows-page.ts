...
function workflowPromptCopyButton(prompt){return prompt?'<button type="button" class="copy-id workflow-prompt-copy" data-copy-value="'+attr(prompt)+'" data-copy-label="Prompt copied" title="Copy prompt" aria-label="Copy prompt"><span class="copy-icon" aria-hidden="true"></span></button>':''}
function workflowPromptCellHtml(prompt,summary){return '<div class="workflow-prompt-cell"><span class="truncate-cell" title="'+attr(summary)+'">'+esc(short(summary||'-',180))+'</span>'+workflowPromptCopyButton(prompt)+'</div>'}
function renderTemplateRow(t){const summary=t.description||t.prompt||'';const tags=workflowTags(t.tags)||'-';const scope='<span class="adapter-status '+(t.scope==='shared'?'enabled':'planned')+'">'+esc(t.scope||'private')+'</span>';const actions=workflowRowActions('<button data-template-run="'+attr(t.id)+'"'+disabledAttr('workflows.run')+'>Run</button>',['<button class="secondary" data-template-insert="'+attr(t.id)+'">Insert</button>','<button class="secondary" data-template-preview="'+attr(t.id)+'">Preview</button>','<button class="secondary" data-template-history="'+attr(t.id)+'">History</button>','<button class="secondary" data-template-export="'+attr(t.id)+'">Export</button>','<button class="secondary" data-template-edit="'+attr(t.id)+'"'+disabledAttr('workflows.write')+'>Edit</button>','<button class="danger" data-template-delete="'+attr(t.id)+'"'+disabledAttr('workflows.write')+'>Delete</button>']);return '<tr>'+workflowCell('Updated',templateUpdatedHtml(t),'updated-cell')+workflowCell('Name','<span class="truncate-cell" title="'+attr(t.name||'')+'">'+esc(short(t.name||'-',120))+'</span>','primary-cell')+workflowCell('Scope',scope,'scope-cell')+workflowCell('Tags',tags,'tags-cell')+workflowCell('Variables',esc(templateVariablesText(t)),'variables-cell')+workflowCell('Prompt',workflowPromptCellHtml(t.prompt||'',summary),'prompt-cell')+workflowCell('Actions',actions,'actions-cell')+'</tr>'}
...
function bindTemplateButtons(){bindUiCopyButtons(document.getElementById('templateList')||document)}
