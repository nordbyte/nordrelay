...
function safe(fn,event=null){if(event&&event.preventDefault)event.preventDefault();Promise.resolve().then(fn).catch(handleUiError)}
loadBootstrap().then(async()=>{await finishBootstrapNavigation();safe(()=>loadQueue({silent:true}));connectEvents()}).catch(handleUiError);
