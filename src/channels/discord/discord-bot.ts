...
export function createDiscordBridge(config: ConnectorConfig, registry: SessionRegistry): DiscordBridge | null {
  ...
  const handlePrompt = async (request: DiscordRequest, input: AgentPromptInput, artifactOutDir?: string, options: { fromQueue?: boolean; attachments?: ReturnType<typeof webChatAttachmentsForStagedFiles> } = {}): Promise<void> => {
  ...
  const drainQueue = async (request: DiscordRequest): Promise<void> => {
    await drainOneQueuedChannelPrompt({
      ...
      scheduleNext: (req) => {
        scheduleQueuedDrain(() => {
          void drainQueue(req).catch((error) => console.error("Failed to drain Discord queue:", error));
