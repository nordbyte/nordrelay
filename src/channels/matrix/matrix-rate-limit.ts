export interface MatrixRateLimiterOptions {
...
export interface MatrixRateLimitMetrics {
  queued: number;
  running: number;
  completed: number;
  failed: number;
...
const DEFAULT_OPTIONS: MatrixRateLimiterOptions = {
  minIntervalMs: 250,
...
export class MatrixRateLimiter {
  ...
  constructor(private options: MatrixRateLimiterOptions = DEFAULT_OPTIONS) {}

  configure(options: Partial<MatrixRateLimiterOptions>): void {
  ...
  getMetrics(): MatrixRateLimitMetrics {
    return {
      queued: this.queued,
...
export const matrixRateLimiter = new MatrixRateLimiter();

export function getMatrixRateLimitMetrics(): MatrixRateLimitMetrics {
  return matrixRateLimiter.getMetrics();
}
...
function getRetryAfterSeconds(error: unknown): number | undefined {
  ...
  const candidate = error as { data?: { retryAfter?: number }; retryAfter?: number; retryAfterMs?: number; headers?: Record<string, string | string[] | undefined> };
  if (typeof candidate.retryAfterMs === "number") {
    return Math.max(1, Math.ceil(candidate.retryAfterMs / 1000));
  }
