...
export function createMatrixBridge(config: ConnectorConfig, registry: SessionRegistry): MatrixBridge | null {
  ...
  const drainQueue = async (request: MatrixRequest): Promise<void> => {
    await drainOneQueuedChannelPrompt({
      ...
      scheduleNext: (req) => {
        scheduleQueuedDrain(() => {
          void drainQueue(req).catch((error) => console.error("Failed to drain Matrix queue:", error));
  ...
  const handleButtonAction = async (request: MatrixRequest, action: string): Promise<void> => {
    ...
    if (peerQueueMatch?.[1] && peerQueueMatch[2]) {
      ...
      await remoteClient.webProxy(peerQueueMatch[1], { method: "POST", path: "/api/queue", body: { action: "cancel", id: peerQueueMatch[2] }, contextKey: sourceContextKey }, actorFor(request), sourceContextKey);
