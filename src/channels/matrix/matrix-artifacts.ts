...
export interface MatrixArtifactRequest {
  contextKey: ChannelContextKey;
  context: ChannelContext;
}

export interface MatrixArtifactCommandDeps<TRequest extends MatrixArtifactRequest> {
...
export function createMatrixArtifactCommandHandler<TRequest extends MatrixArtifactRequest>(
  deps: MatrixArtifactCommandDeps<TRequest>,
  ...
  return async (request, argument) => {
    ...
    if (subcommand === "delivery") {
      ...
      if (!requested) {
        await deps.reply(request, renderArtifactDeliveryAction(deps.getArtifactDeliveryMode?.(request) ?? deps.config.matrixArtifactDeliveryMode, "user").plain);
        return;
      }
      if (requested === "default" || requested === "inherit") {
        const mode = await deps.setArtifactDeliveryMode?.(request, null);
        await deps.reply(request, renderArtifactDeliveryAction(mode ?? deps.config.matrixArtifactDeliveryMode, "user").plain);
        return;
      }
    ...
    if (action) {
      ...
      if (filtered) {
        ...
        await deliverChannelAction(deps.runtime, request.context, renderMatrixArtifactReports(request.contextKey, filtered));
        return;
      }
      ...
      if (shouldZip) {
        await deliverMatrixArtifactZip(deps, request, selected);
      } else {
        await deliverMatrixArtifactReport(deps, request, selected);
      }
      return;
    }

    await deliverChannelAction(deps.runtime, request.context, renderMatrixArtifactReports(request.contextKey, reports));
  };
}

export async function sendRecentMatrixArtifacts<TRequest extends MatrixArtifactRequest>(
  deps: MatrixArtifactCommandDeps<TRequest>,
  request: TRequest,
  ...
  if (policy.includeActions) {
    await deliverChannelAction(deps.runtime, request.context, renderMatrixArtifactReports(request.contextKey, [{
      turnId,
  ...
  if (policy.autoSendZip) {
    ...
    if (bundle) await sendMatrixArtifactFile(deps, request, bundle);
    ...
    for (const artifact of report.artifacts.filter((item) => !policy.imagesOnly || /\.(png|jpe?g|gif|webp)$/i.test(item.name)).slice(0, 5)) {
      await sendMatrixArtifactFile(deps, request, artifact);
    }
  }
  deps.appendActivity(request, {
    status: "info",
...
function renderMatrixArtifactReports(contextKey: ChannelContextKey, reports: ArtifactTurnReport[]) {
  const rendered = renderArtifactReportsAction(reports);
  return {
    ...rendered,
    buttons: reports.slice(0, 5).map((report, index) => [
      { label: `${index + 1} Send`, action: `matrix_artifact_send:${contextKey}:${report.turnId}` },
      { label: `${index + 1} ZIP`, action: `matrix_artifact_zip:${contextKey}:${report.turnId}` },
      { label: `${index + 1} Delete`, action: `matrix_artifact_delete:${contextKey}:${report.turnId}` },
    ]),
  };
}
...
async function deliverMatrixArtifactZip<TRequest extends MatrixArtifactRequest>(
  deps: MatrixArtifactCommandDeps<TRequest>,
  request: TRequest,
  ...
  if (!deps.runtime.sendFile) {
    await deps.reply(request, "This Matrix runtime cannot send artifact files.");
    return;
  }
...
async function deliverMatrixArtifactReport<TRequest extends MatrixArtifactRequest>(
  deps: MatrixArtifactCommandDeps<TRequest>,
  request: TRequest,
  report: ArtifactTurnReport,
): Promise<void> {
  ...
  for (const artifact of delivered) {
    if (!await sendMatrixArtifactFile(deps, request, artifact)) {
      failedCount += 1;
    }
  }
...
async function sendMatrixArtifactFile<TRequest extends MatrixArtifactRequest>(
  deps: MatrixArtifactCommandDeps<TRequest>,
  request: TRequest,
  artifact: Artifact,
): Promise<boolean> {
  ...
  try {
    ...
    console.error(`Failed to send Matrix artifact ${artifact.name}:`, error);
    return false;
  }
}
