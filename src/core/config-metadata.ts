...
export const SETTING_DEFINITIONS: SettingDefinition[] = [
  ...
  setting(
    "NORDRELAY_AUTOSTART_ENABLED",
    "Auto-start NordRelay",
    "Dashboard",
    "boolean",
    "Create or remove a user-level autostart entry for the NordRelay connector.",
    false,
    undefined,
    "On Linux this manages a systemd user service, on macOS a LaunchAgent, and on Windows a Task Scheduler entry.",
  ),
  setting(
    "NORDRELAY_WEBUI_AUTOSTART_ENABLED",
    "Auto-start WebUI",
    "Dashboard",
    "boolean",
    "Create or remove a user-level autostart entry for the NordRelay WebUI dashboard.",
    false,
    undefined,
    "The WebUI autostart runs `nordrelay web`, which also starts the connector when needed.",
  ),
  setting("NORDRELAY_WEBAUTHN_ENABLED", "Enable passkeys", "Security", "boolean", "Allow WebAuthn/passkey registration and login in the WebUI.", true),
  setting("NORDRELAY_WEBAUTHN_RP_NAME", "Passkey RP name", "Security", "string", "Display name shown by browsers during passkey registration.", true),
  setting("NORDRELAY_WEBAUTHN_RP_ID", "Passkey RP ID", "Security", "string", "Optional relying-party domain. Leave empty to use the dashboard hostname.", true),
  ...
  setting(
    "CODEX_EXTERNAL_APPROVAL_CONTROL",
    "External approval control",
    "Codex",
    "boolean",
    "Allow NordRelay to submit approval choices to a trusted local Codex CLI terminal when the OS permits TTY input control.",
    true,
    undefined,
    "Leave disabled unless this machine is trusted. Detection and display still work when disabled; only remote approval button submission is blocked.",
  ),
  setting(
    "CODEX_EXTERNAL_APPROVAL_SUDO_HELPER",
    "External approval sudo helper",
    "Codex",
    "boolean",
    "Allow NordRelay to fall back to a passwordless sudo helper for local Codex CLI terminal input when same-user TTY injection is denied.",
    true,
    undefined,
    "This is for trusted local machines only. It requires sudo -n python3 to work and never runs through a shell.",
  ),
  ...
  setting("NORDRELAY_ARTIFACTS_ENABLED", "Enable artifact tracking", "Artifacts", "boolean", "Scan and persist generated artifact reports after turns. Disabled by default; enable only when needed.", false),
  setting("NORDRELAY_AUTO_SEND_ARTIFACTS", "Default auto-send artifacts", "Artifacts", "boolean", "Default automatic artifact summaries/uploads for chat adapters.", false),
  setting("NORDRELAY_ARTIFACT_DELIVERY", "Default artifact delivery", "Artifacts", "string", "Default artifact delivery mode for chat adapters.", false, ["manual-only", "summary", "summary-with-actions", "auto-files", "auto-zip", "images-only", "off"]),
  ...
  setting("NORDRELAY_ACTIVE_DISCOVERY_CACHE_TTL_MS", "Active discovery cache TTL", "Workspace", "number", "TTL for active-session discovery snapshots across Codex, Pi, Hermes, Claude Code, and recorded sessions. Set 0 to disable this cache.", true),
  setting("NORDRELAY_OPENCLAW_ACTIVE_DISCOVERY_CACHE_TTL_MS", "OpenClaw active discovery TTL", "Workspace", "number", "TTL for OpenClaw active-session discovery. Use a higher value when OpenClaw discovery is slow; set 0 to disable this cache.", true),
  ...
  setting("FFMPEG_PATH", "ffmpeg path", "Voice", "string", "Optional absolute ffmpeg executable path used when service/autostart PATH cannot find ffmpeg.", true),
...
const EXAMPLE_VALUES: Record<string, string> = {
  ...
  "NORDRELAY_AUTOSTART_ENABLED": "false",
  "NORDRELAY_WEBUI_AUTOSTART_ENABLED": "false",
  "NORDRELAY_WEBAUTHN_ENABLED": "true",
  "NORDRELAY_WEBAUTHN_RP_NAME": "NordRelay",
  "NORDRELAY_WEBAUTHN_RP_ID": "",
  "NORDRELAY_WEBAUTHN_ORIGIN": "",
  "NORDRELAY_PLUGINS_ENABLED": "true",
  "NORDRELAY_PLUGIN_GITHUB_INSTALL_ENABLED": "true",
  "NORDRELAY_PLUGIN_ALLOW_BUILD_SCRIPTS": "false",
  ...
  "CODEX_EXTERNAL_APPROVAL_CONTROL": "false",
  "CODEX_EXTERNAL_APPROVAL_SUDO_HELPER": "false",
  ...
  "NORDRELAY_ACTIVE_DISCOVERY_CACHE_TTL_MS": "5000",
  "NORDRELAY_OPENCLAW_ACTIVE_DISCOVERY_CACHE_TTL_MS": "120000",
...
const GROUP_INTROS: Record<string, string> = {
  ...
  Plugins: "Local and GitHub plugin management. Plugins are explicit installs, permission-gated, and disabled unless enabled by an admin.",
};
