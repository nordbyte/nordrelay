...
export function activityCategoryForType(type: string): WebActivityCategory {
  ...
  if (/^(user_|group_|telegram_chat_|telegram_link|discord_channel_|discord_link|slack_channel_|slack_link|matrix_room_|matrix_link|peer_|plugin_|permission_|access_|lock_)/.test(type)) return "security";
...
export function auditCategoryForAction(action: string): WebActivityCategory {
  ...
  if (/^(permission_|user_|group_|telegram_|discord_|slack_|matrix_|peer_|plugin_)/.test(action)) return "security";
