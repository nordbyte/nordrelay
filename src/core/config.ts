...
export interface ConnectorConfig {
  ...
  codexExternalApprovalSudoHelper: boolean;
...
export function loadConfig(): ConnectorConfig {
  ...
  const codexExternalApprovalSudoHelper = parseBooleanEnv(optionalString(process.env.CODEX_EXTERNAL_APPROVAL_SUDO_HELPER), false);
  ...
  const openClawActiveDiscoveryCacheTtlMs = parseNonNegativeIntegerEnv(optionalString(process.env.NORDRELAY_OPENCLAW_ACTIVE_DISCOVERY_CACHE_TTL_MS), 120_000, "NORDRELAY_OPENCLAW_ACTIVE_DISCOVERY_CACHE_TTL_MS");
  ...
  return {
    ...
    codexExternalApprovalSudoHelper,
    codexSandboxMode,
    codexApprovalPolicy,
    launchProfiles,
