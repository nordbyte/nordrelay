...
function appendMessage(cls,text){const box=document.getElementById('messages');const div=document.createElement('div');div.className='message '+cls;const body=document.createElement('div');body.className='message-body';div.appendChild(body);setMessageText(div,text);box.appendChild(div);scrollChatToBottom();return div}
...
function setMessageText(div,text){div.__rawText=String(text??'');const body=messageBody(div);body.innerHTML=renderChatMarkdown(div.__rawText);bindChatCopyButtons(body);return div}
function appendQueuedMessage(id){const div=appendMessage('system','Queued prompt '+id);const body=messageBody(div);const btn=document.createElement('button');btn.textContent='Cancel queued message';btn.className='danger';btn.onclick=()=>safe(async()=>{const r=await api('/api/queue',{method:'POST',body:JSON.stringify({action:'cancel',id})});renderQueue(r.queue,r.paused);setMessageText(div,'Cancelled queued prompt '+id)});body.appendChild(document.createElement('br'));body.appendChild(btn)}
function renderChatMessages(messages){state.chatMessages=messages||[];const box=document.getElementById('messages');box.innerHTML=(messages||[]).map(m=>'<div class="message '+esc(m.role)+'"><small>'+esc((m.source||'web')+' / '+fmtDate(m.timestamp))+'</small><br><div class="message-body">'+renderChatMarkdown(m.text)+'</div></div>').join('');bindChatCopyButtons(box);scrollChatToBottom()}
async function loadChatHistory(){const data=await api('/api/chat/history');renderChatMessages(data.messages||[])}
let currentAgentMessage=null;
function renderChatMarkdown(text){let output=esc(String(text??''));const codeBlocks=[];const inlineCode=[];output=extractChatCodeBlocks(output,codeBlocks);output=extractChatInlineCode(output,inlineCode);output=formatChatBold(output);output=formatChatItalic(output);output=formatChatLinks(output);output=formatChatBlockquotes(output);output=formatChatLists(output);output=formatChatHeadings(output);output=restoreChatMarkdown(output,CHAT_INLINE_CODE_PREFIX,CHAT_INLINE_CODE_SUFFIX,inlineCode);output=restoreChatMarkdown(output,CHAT_CODE_BLOCK_PREFIX,CHAT_CODE_BLOCK_SUFFIX,codeBlocks);return output}
function extractChatCodeBlocks(text,blocks){return text.replace(/```([^\n`]*)\n?([\s\S]*?)```/g,(_match,rawLanguage,rawCode)=>{const language=String(rawLanguage||'').trim().replace(/[^a-zA-Z0-9_+-]/g,'');const className=language?' class="language-'+attr(language)+'"':'';const label=language?' data-code-language="'+attr(language)+'"':'';const block='<pre class="chat-code-block" tabindex="0" title="Copy code" data-chat-copy="code-block"'+label+'><code'+className+'>'+rawCode+'</code></pre>';const index=blocks.push(block)-1;return CHAT_CODE_BLOCK_PREFIX+index+CHAT_CODE_BLOCK_SUFFIX})}
function extractChatInlineCode(text,inline){let result='';let index=0;while(index<text.length){if(text[index]!=='`'){result+=text[index];index+=1;continue}let tickCount=1;while(text[index+tickCount]==='`')tickCount+=1;const fence='`'.repeat(tickCount);const start=index+tickCount;const end=text.indexOf(fence,start);if(end===-1){result+=fence;index+=tickCount;continue}const content=text.slice(start,end);if(content.includes('\n')){result+=fence;index+=tickCount;continue}const button='<button type="button" class="chat-inline-code" title="Copy code" data-chat-copy="inline-code">'+content+'</button>';result+=CHAT_INLINE_CODE_PREFIX+(inline.push(button)-1)+CHAT_INLINE_CODE_SUFFIX;index=end+tickCount}return result}
...
function formatChatLists(text){const lines=text.split('\n');const out=[];let list=null;const flush=()=>{if(list){out.push('</'+list+'>');list=null}};for(const line of lines){const task=line.match(/^\s*[-*]\s+\[([ xX])\]\s+(.+)$/);const bullet=line.match(/^\s*[-*]\s+(.+)$/);const ordered=line.match(/^\s*\d+[.)]\s+(.+)$/);if(task){if(list!=='ul'){flush();out.push('<ul class="chat-list chat-task-list">');list='ul'}const checked=task[1].toLowerCase()==='x'?'[x]':'[ ]';out.push('<li><span class="chat-task-mark">'+checked+'</span> '+task[2]+'</li>');continue}if(bullet){if(list!=='ul'){flush();out.push('<ul class="chat-list">');list='ul'}out.push('<li>'+bullet[1]+'</li>');continue}if(ordered){if(list!=='ol'){flush();out.push('<ol class="chat-list">');list='ol'}out.push('<li>'+ordered[1]+'</li>');continue}flush();out.push(line)}flush();return out.join('\n')}
...
function bindChatCopyButtons(root){root.querySelectorAll?.('[data-chat-copy]').forEach(el=>{el.onclick=event=>{event.preventDefault();event.stopPropagation();const code=el.dataset.chatCopy==='code-block'?el.querySelector('code')?.textContent:el.textContent;copyText(code||'','Copied code')};el.onkeydown=event=>{if(el.tagName==='BUTTON')return;if(event.key==='Enter'||event.key===' '){event.preventDefault();el.click()}}})}
function connectEvents(){
  ...
  events.addEventListener('text_delta', e=>{const d=JSON.parse(e.data);if(!currentAgentMessage)currentAgentMessage=appendMessage('agent','');setMessageText(currentAgentMessage,(currentAgentMessage.__rawText||'')+d.delta);scrollChatToBottom();if(state.currentPage==='tasks')loadTasks()});
