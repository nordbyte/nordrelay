...
describe("stageFile", () => {
  const testDir = path.join(tmpdir(), `nordrelay-test-${randomUUID()}`);
...
describe("cleanupInbox", () => {
  it("removes the inbox directory without throwing", async () => {
    const dir = path.join(tmpdir(), `nordrelay-cleanup-${randomUUID()}`);
  ...
  it("does not throw when the directory does not exist", async () => {
    const dir = path.join(tmpdir(), `nordrelay-missing-${randomUUID()}`);
