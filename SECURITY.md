...
Do not include real Telegram, Discord, Slack, or Matrix bot tokens, OpenAI API keys, Codex credentials, local rollout files, or private source code in reports.
...
Treat enabling `danger-full-access`, broad write permissions, Telegram group chat access, Discord guild-channel access, Slack channel access, Matrix room access, or peer access with prompt/write scopes as equivalent to granting remote shell-adjacent control over the host.
